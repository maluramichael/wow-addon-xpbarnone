...
local function GetOptions(uiTypes, uiName, appName)
    if appName == "XPBarNone-General" then
        local options = {
            type = "group",
            name = C_AddOns.GetAddOnMetadata("XPBarNone", "Title"),
            ...
            args = {
                xpbndesc = {
                    type = "description",
                    order = 0,
                    name = C_AddOns.GetAddOnMetadata("XPBarNone", "Notes"),
                },
                locked = {
                    name = L["Lock"],
...
function XPBarNone:OnInitialize()
    ...
    -- Register options
    local myName = C_AddOns.GetAddOnMetadata("XPBarNone", "Title")
    ...
    self:RegisterChatCommand("xpbn", function() Settings.OpenToCategory(myName) end)
    ...
    self.frame.button:SetScript("OnClick", function(clickself, button, down)
        ...
        if IsShiftKeyDown() and button == "RightButton" then
            Settings.OpenToCategory(
                C_AddOns.GetAddOnMetadata("XPBarNone", "Title")
            )
        end
